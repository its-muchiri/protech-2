// Fix apostrophes in title/description - use correct regex from lib
const fs = require('fs');
...
for (const f of files) {
  ...
  // Use EXACT regex from lib/blog-content.js - match[1] = inner fm, match[2] = body
  const match = raw.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n([\s\S]*)$/);
  if (!match) {
    failed.push({ file: f, reason: 'no frontmatter match' });
    continue;
  }
  
  let fmText = match[1];  // Already inner content, no delimiters
  const body = match[2];
  ...
  // Also handle: key: "value" patterns
  fmText = fmText.replace(/^(\w+:)\s*"([^"]*)"$/gm, (m, key, val) => {
    return key + " '" + val.replace(/'/g, "''") + "'";
  });
