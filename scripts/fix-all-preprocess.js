// Pre-process frontmatter text to fix brackets/apostrophes BEFORE YAML parsing
const fs = require('fs');
...
for (const f of files) {
  ...
  // PRE-PROCESS: fix frontmatter text BEFORE YAML parsing
  // 1. Remove [brackets]
  fmText = fmText.replace(/\[[^\]]+\]/g, '');
  // 2. Escape apostrophes in single-quoted values: key: 'value with 'apostrophe''
  fmText = fmText.replace(/^(\s*\w+:\s*)'([^']*)'$/gm, (m, prefix, val) => {
    return prefix + "'" + val.replace(/'/g, "''") + "'";
  });
  // 3. Handle unquoted description/primary_keyword with apostrophes
  fmText = fmText.replace(/^(\s*(description|primary_keyword):\s*)([^'"][^\n]*[^'"])$/gm, (m, prefix, key, val) => {
    return prefix + "'" + val.trim().replace(/'/g, "''") + "'";
  });
  // 4. Handle already-quoted values with apostrophes
  fmText = fmText.replace(/^(\s*\w+:\s*)'([^']*(?:'[^']*)*)'$/gm, (m, prefix, val) => {
    return prefix + "'" + val.replace(/'/g, "''") + "'";
  });
  ...
  // Clean all string fields: remove any remaining brackets, escape apostrophes
  const clean = (val) => {
